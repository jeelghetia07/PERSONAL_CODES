#include<bits/stdc++.h>
using namespace std;

/*      LC = 781
    There is a forest with an unknown number of rabbits. We asked n rabbits "How many other rabbits have the same color as you?" and collected the answers in an integer array answers where answers[i] is the answer of the ith rabbit.

    Given the array answers, return the minimum number of rabbits that could be in the forest.
*/

class Solution {
public:
    int calc(int num, int freq){
        int groupSize = (num + 1);      // practically ((num*freq)/freq) + 1;
        int No_of_Groups = (freq + groupSize - 1) / groupSize;

        return (groupSize * No_of_Groups);
    }
    int numRabbits(vector<int>& arr) {
        int n = arr.size();
        unordered_map<int, int> mpp;

        for(int i = 0 ; i < n ; i++){
            mpp[arr[i]]++;
        }

        int totalRabbits = 0;

        for(auto it : mpp){
            int num = it.first;
            int freq = it.second;

            totalRabbits += calc(num, freq);
        }

        return totalRabbits;
    }
};

int main(){
    /*
        Input: answers = [1,1,2]
        Output: 5
        Explanation:
        The two rabbits that answered "1" could both be the same color, say red.
        The rabbit that answered "2" can't be red or the answers would be inconsistent.
        Say the rabbit that answered "2" was blue.
        Then there should be 2 other blue rabbits in the forest that didn't answer into the array.
        The smallest possible number of rabbits in the forest is therefore 5: 3 that answered plus 2 that didn't.

        Example 2:
        Input: answers = [10,10,10]
        Output: 11
    */

    return 0;
}