...
public:
    vector<int> survivedRobotsHealths(vector<int>& positions, vector<int>& healths, string directions) {
        ...
        vector<int> order(n);
        iota(order.begin(), order.end(), 0);        // this fills the array like [0,1,2,....n-1]
        ...
        for(int i : order){
            if(directions[i] == 'R'){
                ...
                while(!st.empty()){
                    int topEle = st.top();
                    if(healths[topEle] < healths[i]){
                        alive[topEle] = false;
                        st.pop();
                        healths[i]--;
                    }
                    else if(healths[topEle] > healths[i]){
                        alive[i] = false;
                        healths[topEle]--;
                        break;
                    }
        ...
        for(int i = 0 ; i < n ; i++){           // as said we have to maintain the order, we iterate normally.
